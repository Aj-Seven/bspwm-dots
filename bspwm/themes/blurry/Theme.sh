...
set_picom_config() {
		sed -i "$HOME"/.config/bspwm/picom.conf \
            -e "s/blur-method = .*/blur-method = \""dual_kawase"\";/g" \
			-e "s/normal = .*/normal =  { fade = true; shadow = true; }/g" \
			-e "s/shadow-color = .*/shadow-color = \"#000000\"/g" \
			-e "s/corner-radius = .*/corner-radius = 7/g" \
			-e "s/\".*:class_g = 'Alacritty'\"/\"70:class_g = 'Alacritty'\"/g" \
			-e "s/\".*:class_g = 'FloaTerm'\"/\"75:class_g = 'FloaTerm'\"/g" \
            -e "s/\".*:class_g = 'firefox'\"/\"70:class_g = 'firefox'\"/g" 
}
...
# Launch the bar
launch_bars() {
		polybar -c $HOME/.config/bspwm/themes/blurry/config.ini &
}
